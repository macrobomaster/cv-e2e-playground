...
while True:
  ...
  x = session.run(
    None,
    {
      ...
      "color": np.array([[0]], dtype=np.int32) if color == "red" else np.array([[1]], dtype=np.int32),
    },
  )[0]
  ...
  if color == "red": cv2.putText(frame, "detecting red", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (55, 250, 55), 2)
  elif color == "blue": cv2.putText(frame, "detecting blue", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (55, 250, 55), 2)

  cv2.imshow("preview", cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))

  key = cv2.waitKey(1)
  if key == ord("q"): break
  elif key == ord("r"): color = "red"
  elif key == ord("b"): color = "blue"
